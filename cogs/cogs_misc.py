...
class Misc(commands.Cog):
    ...
    async def roll(self, ctx, input:str='1d2'):

        auxiliar = Auxiliar(self.bot)

        match = re.match(r'(?:(\d+)#)?(\d+)d(\d+)([+-]\d+)?', input)
        if not match:
            auxiliar.send_embed_message(ctx, 'Input should be in the format "6#4d6"')
            return

        repeat = int(match.group(1)) if match.group(1) else 1
        dice = int(match.group(2))
        faces = int(match.group(3))
        sum = int(match.group(4)) if match.group(4) else 0

        formatted = ""

        if repeat == 1 and dice == 1 and faces == 2 and sum == 0:
            rng = rn(1,(faces))
        ...
        for x in range(repeat):
            ...
            if sum > 0:
                formatted += f"] {dice}d{faces} + {sum}"
            elif sum < 0:
                formatted += f"] {dice}d{faces} - {sum*(-1)}"
            else:
